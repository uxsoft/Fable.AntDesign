...
type AntSelect<'T>() =
    inherit AntElement<AntSelect<'T>>(ofImport "Select" "antd")
    ...
    member x.filterOption (v: Func<string, 'T, bool>) = x.attribute "filterOption" v
    member x.filterSort (v: Func<'T, 'T, int>) = x.attribute "filterSort" v
    ...
    member x.options (v: 'T array) = x.attribute "options" v
    ...
    member x.tagRender (v: Func<'T, ReactElement>) = x.attribute "tagRender" v
